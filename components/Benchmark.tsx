...
export const Benchmark: React.FunctionComponent<BenchmarkProps> = ({
  ...
  const difference =
    avgUnistyles === 'N/A' || avgStyleSheet === 'N/A'
      ? 'N/A'
    ...
    <ScrollView
      ...
      <View style={styles.resultsRow}>
        ...
        <View style={styles.wrap}>
          {benchmark.renderTime[Library.StyleSheet].map((time, index) => (
      ...
      <View style={styles.resultsRow}>
        <Text style={styles.libName}>Unistyles</Text>
        <View style={styles.wrap}>
          {benchmark.renderTime[Library.Unistyles].map((time, index) => (
            <Text key={index}>{`${index + 1}) ${time}ms `}</Text>
          ))}
        </View>
        <Text style={styles.result}>Avg: {avgUnistyles}ms</Text>
      </View>
      <Text
        style={{
          ...styles.difference,
      ...
      <Text style={styles.difference}>
        Cost per view:{' '}
        {difference === 'N/A' ? 'N/A' : `+${(parseFloat(difference) / boxes).toFixed(4)} ms`}
      </Text>
